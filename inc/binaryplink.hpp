...
class BinaryPlink: public Genotype
{
    ...
    private:
                ...
                const std::string &file_name)
        {
            ...
            if (fseeko(m_bedfile, m_bed_offset+(snp_index*((uint64_t)unfiltered_sample_ct4)), SEEK_SET))
            {
            ...
            // this is for LD calculation, thus doesn't really need to worry about the founder_count
            // otherwise we need to use the sample_ct instead
            // This is also ok for the LD reference file as the only way that will affect the
            // sample inclusion / exclusion is already handled during file loading
