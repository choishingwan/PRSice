...
public:
    ...
    void init_prslice_process_count(size_t num_prslice_region, size_t num_thresholds)
        {
            // we first run PRSice within each region
            m_total_process = num_thresholds * num_phenotype() * num_prslice_region+
    ...
    void get_prslice_best_threshold(const Commander& commander, Genotype& target, const size_t num_prslice_regions, const size_t pheno_index);
