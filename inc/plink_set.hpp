...
typedef struct
{
    ...
    //   [2]: number of bits (divisible by 128 unless a variant in the last
    //   block
    //        is included)
...
uint32_t setdef_iter(uint32_t* setdef, uint32_t* cur_idx_ptr,
                     uint32_t* aux_ptr);
...
int32_t annotate(Annot_info* aip, char* outname, char* outname_end,
                 double pfilter, Chrom_info* chrom_info_ptr);
