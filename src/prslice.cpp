...
/*
void PRSice::prslice_windows(const Commander &c_commander, const Region
&c_region)
{
    std::string prev_chr = "";
    size_t prev_loc = 0;
    ...
    double bound_start =  (fastscore)? c_commander.get_bar_lower():
c_commander.get_lower(); double bound_end = (fastscore)?
c_commander.get_bar_upper():c_commander.get_upper(); double bound_inter =
c_commander.get_inter(); std::unordered_map<std::string, std::string> chr_files;
    ...
    for(auto snp : m_snp_list)
    {
        if(m_include_snp.find(snp.get_rs_id())!=m_include_snp.end())
        {
            ...
            if(m_partition.size()!=0 && (prev_chr.empty() || cur_chr != prev_chr
|| cur_loc - prev_loc > wind_size))
            {
...
cur_chr+":"+std::to_string(m_snp_list[std::get<+PRS::INDEX>(m_partition.front())].get_loc())
+
...
std::get<+PRS::P>(m_best_threshold[0]); std::get<prslice_wind::NSNP>(wind) =
std::get<+PRS::NSNP>(m_best_threshold[0]); std::get<prslice_wind::COEFF>(wind) =
std::get<+PRS::COEFF>(m_best_threshold[0]); m_best_snps.push_back(wind);
...
(int)((bound_end+0.1)-bound_start)/bound_inter; // This ensure they all fall
into the same category m_partition.push_back(part);
                    partition_index[snp.get_rs_id()] = m_partition.size()-1;
                }
            }
        }
    ...
    if(m_partition.size()!=0)
    {
        ...
        update_line(partition_index);
        std::string window_name =
m_snp_list[front_index].get_chr()+":"+std::to_string(m_snp_list[front_index].get_loc())
+
                "-"+std::to_string(m_snp_list[std::get<+PRS::INDEX>(m_partition.back())].get_loc());
        size_t num_snps = std::get<+PRS::NSNP>(m_best_threshold[0]);
        double best_r2 = std::get<+PRS::R2>(m_best_threshold[0]);
...
std::get<+PRS::COEFF>(m_best_threshold[0]); m_best_snps.push_back(wind);
                      ...
                      [](windows const &t1, windows const &t2)
            {
                ...
                {
                    return
std::get<prslice_wind::WIND>(t1).compare(std::get<prslice_wind::WIND>(t2))<0;
                }
                else return std::get<prslice_wind::R2>(t1)
<std::get<prslice_wind::R2>(t2);
            }
                     );
}

void PRSice::prslice(const Commander &c_commander, const Region &c_region, const
size_t c_pheno_index)
{
    ...
    {
        m_partition.insert(m_partition.end(),
std::get<prslice_wind::SNPS>(snps).begin(),
std::get<prslice_wind::SNPS>(snps).end());
        //PLINK prs(m_target, m_chr_list, m_score);
        //prs.initialize();
        // after this, the m_current_prs[i_region] will be updated
        //prs.get_score(m_partition, m_snp_list, m_current_prs, cur_index,
m_partition.size()); cur_index=m_partition.size(); m_num_snp_included.front() +=
m_partition.size(); thread_score(0, 1, bin_count,n_thread, c_pheno_index);
        bin_count++;
    }
}
...
std::get<pheno_store::NAME>(m_pheno_names[pheno_index]); std::string
output_prefix = c_commander.get_out()+"."+m_base_name; if(!pheno_name.empty())
output_prefix.append("."+pheno_name+"."); std::string out_best =
output_prefix+".best"; std::string out_prslice = output_prefix+".prslice";
    ...
    if(!prslice.is_open())
    {
        std::string error_message = "Cannot open file " +out_prslice+" for
write"; throw std::runtime_error(error_message);
    }
    if(!window.is_open())
    {
    ...
    best_prs << "IID\tprs_"<<std::get<+PRS::THRESHOLD>(m_best_threshold[0]) <<
std::endl; prslice << "Num_Windows\tR2\tCoefficient\tP\tNum_SNP" << std::endl;
...
void PRSice::update_line(std::unordered_map<std::string, size_t>
&partition_index)
{
    std::ifstream bim;
    ...
    if(!bim.is_open())
    {
        std::string error_message = "Cannot open bim file " + bim_name+" for
PRSlice"; throw std::runtime_error(error_message);
    }
    std::string line;
    size_t cur_line = 0;
    while(std::getline(bim, line))
    {
        ...
        if(!line.empty())
        {
            ...
            if(token.size() < 6) throw std::runtime_error("Malformed bim file,
should contain at least 6 columns");
            if(partition_index.find(token[+BIM::RS])!=partition_index.end())
            {
                std::get<+PRS::LINE>(m_partition[partition_index[token[+BIM::RS]]])
=cur_line;
            }
            cur_line++;
        }
