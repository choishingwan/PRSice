                                  ...
                                  Genotype& reference)
{
    ...
    for (size_t i_snp = 0; i_snp < num_snp; ++i_snp)
    {
             ...
             ++clump_idx)
        {
            ...
            { continue; }
            assert(unfiltered_sample_ctv2 * (clump_geno_idx + 1)
                   < geno_storage.size());
            reference.read_genotype(
                &geno_storage[clump_geno_idx * unfiltered_sample_ctv2],
                clump_snp.get_byte_pos(true), clump_snp.get_file_idx(true));
            ++clump_geno_idx;
        }
        // now read in core snppanel
        reference.read_genotype(
            &geno_storage[clump_geno_idx * unfiltered_sample_ctv2],
            core_snp.get_byte_pos(true), core_snp.get_file_idx(true));
        update_index_tot(
            founder_ctl2, founder_ctv2, reference.m_founder_ct, index_data,
            index_tots, founder_include2,
            &geno_storage[clump_geno_idx * unfiltered_sample_ctv2]);
        clump_geno_idx = 0;
        for (size_t clump_idx = clump_start_idx; clump_idx < core_snp_idx;
             ++clump_idx)
