                            ...
                            bool& error, std::string& error_message)
{
    if (!clumping.no_clump) {
        ...
        if (reference_panel.type.compare("bgen") == 0
            || (reference_panel.file_name.empty() && reference_panel.multi_name.empty()
                && target.type.compare("bgen") == 0))
        {
            ...
            else if (!reference_panel.file_name.empty() || !reference_panel.multi_name.empty())
            {
                             ...
                             bool& error, std::string& error_message)
{
    if (target.name.empty() && target.multi_name.empty()) {
        error = true;
        error_message.append("Error: You must provide a target file or a file containing all target prefixs!\n");
    }
