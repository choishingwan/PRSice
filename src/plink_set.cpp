...
uint32_t in_setdef(uint32_t* setdef, uint32_t marker_idx)
{
    ...
    if (range_ct != 0xffffffffU) {
        ...
        return (
            uint32arr_greater_than(&(setdef[1]), range_ct * 2, marker_idx + 1)
            & 1);
    }
    else
    {
...
uint32_t in_setdef_dist(uint32_t* setdef, uint32_t pos, uint32_t border,
                        int32_t* dist_ptr)
{
    uint32_t range_ct = setdef[0];
    uint32_t uii;
    ...
    else
    {
        ...
        // ties broken in favor of negative numbers, to match PLINK 1.07
        // annot.cpp
        if (setdef[uii] + border > pos) {
            ii = pos + 1 - setdef[uii];
...
uint32_t interval_in_setdef(uint32_t* setdef, uint32_t marker_idx_start,
                            uint32_t marker_idx_end)
{
    ...
    if (range_ct != 0xffffffffU) {
        ...
        uii = uint32arr_greater_than(&(setdef[1]), range_ct * 2,
                                     marker_idx_start + 1);
        if (uii & 1) {
            return 1;
        }
        ...
        return uint32arr_greater_than(&(setdef[1 + uii]), range_ct * 2 - uii,
                                      marker_idx_end);
    }
    else
    {
        idx_base = setdef[1];
        if ((marker_idx_end <= idx_base)
            || (marker_idx_start >= idx_base + setdef[2]))
        {
            return setdef[3];
        }
        ...
        uii = next_set((uintptr_t*) (&(setdef[4])), marker_idx_start,
                       marker_idx_end);
...
uint32_t setdef_size(uint32_t* setdef, uint32_t marker_ct)
{
    ...
    else
    {
        ...
        total_size +=
            popcount_bit_idx((uintptr_t*) (&(setdef[4])), 0, setdef[2]);
    }
    return total_size;
}

void setdef_iter_init(uint32_t* setdef, uint32_t marker_ct, uint32_t start_idx,
                      uint32_t* cur_idx_ptr, uint32_t* aux_ptr)
{
    uint32_t range_ct = setdef[0];
    uint32_t uii;
    if (range_ct != 0xffffffffU) {
...
uint32_t setdef_iter(uint32_t* setdef, uint32_t* cur_idx_ptr, uint32_t* aux_ptr)
{
    ...
    // (after the first call) cur_idx is incremented right before this is
    // called.
    uint32_t range_ct = setdef[0];
...
uint32_t alloc_and_populate_nonempty_set_incl(Set_info*   sip,
...
int32_t load_range_list(FILE* infile, uint32_t track_set_names,
                        uint32_t border_extend, uint32_t collapse_group,
                        uint32_t fail_on_no_sets, uint32_t c_prefix,
                        uint32_t allow_no_variants, uintptr_t subset_ct,
                        char* sorted_subset_ids, uintptr_t max_subset_id_len,
                        uint32_t* marker_pos, Chrom_info* chrom_info_ptr,
                        uintptr_t* set_ct_ptr, char** set_names_ptr,
                        uintptr_t*        max_set_id_len_ptr,
                        Make_set_range*** make_set_range_arr_ptr,
                        uint64_t** range_sort_buf_ptr, const char* file_descrip)
{
    ...
    uint32_t         ujj;
    {
        ...
        if (track_set_names) {
            while (fgets(g_textbuf, MAXLINELEN, infile)) {
                ...
                if (!g_textbuf[MAXLINELEN - 1]) {
                    sprintf(g_logbuf,
                            "Error: Line %" PRIuPTR
                            " of %s file is pathologically long.\n",
                ...
                if (no_more_tokens_kns(bufptr3)) {
                    sprintf(g_logbuf,
                            "Error: Line %" PRIuPTR
                            " of %s file has fewer tokens than expected.\n",
                            line_idx, file_descrip);
                    goto load_range_list_ret_INVALID_FORMAT_2;
                }
                const uint32_t chrom_name_slen =
                    (uintptr_t)(first_token_end - textbuf_first_token);
                *first_token_end       = '\0';
                int32_t cur_chrom_code = get_chrom_code(
                    textbuf_first_token, chrom_info_ptr, chrom_name_slen);
                if (cur_chrom_code < 0) {
                    sprintf(g_logbuf,
                            "Error: Invalid chromosome code on line %" PRIuPTR
                            " of %s file.\n",
                            line_idx, file_descrip);
                    goto load_range_list_ret_INVALID_FORMAT_2;
                }
                ...
                bufptr2[uii] = '\0';
                if (subset_ct) {
                    if (bsearch_str(bufptr2, uii, sorted_subset_ids,
                                    max_subset_id_len, subset_ct)
                        == -1)
                    {
                        continue;
                    }
                ...
                // when there are repeats, they are likely to be next to each
                // other
                if (make_set_ll && (!strcmp(make_set_ll->ss, bufptr3))) {
                    continue;
                }
                uii++;
                // argh, --clump counts positional overlaps which don't include
                // any variants in the dataset.  So we prefix set IDs with a
                // chromosome index in that case (with leading zeroes) and treat
                // cross-chromosome sets as distinct.
                if (!marker_pos) {
                    uii += 4;
                }
                ...
                else
                {
                    ...
                    // if first character of gene name is a digit, natural sort
                    // has strange effects unless we force [3] to be
                    // nonnumeric...
                    ll_tmp->ss[3] -= 15;
                ...
                if (fail_on_no_sets) {
                    if (marker_pos) {
                        if (!allow_no_variants) {
                            ...
                            logerrprint("Error: All variants excluded by "
                                        "--gene{-all}, since no sets were "
                                        "defined from\n--make-set file.\n");
                    ...
                    {
                        if (subset_ct) {
                            logerrprint("Error: No --gene-subset genes present "
                                        "in --gene-report file.\n");
                        }
                        else
                        {
            ...
            if (max_set_id_len > MAX_ID_BLEN) {
                logerrprint("Error: Set IDs are limited to " MAX_ID_SLEN_STR
                            " characters.\n");
            ...
            if (!c_prefix) {
                for (ulii = 0; ulii < set_ct; ulii++) {
                    strcpy(&(set_names[ulii * max_set_id_len]),
                           make_set_ll->ss);
            ...
            else
            {
                for (ulii = 0; ulii < set_ct; ulii++) {
                    ...
                    strcpy(&(set_names[ulii * max_set_id_len + 2]),
                           make_set_ll->ss);
                    make_set_ll = make_set_ll->next;
                }
            }
            qsort(set_names, set_ct, max_set_id_len, strcmp_natural);
            set_ct = collapse_duplicate_ids(set_names, set_ct, max_set_id_len,
                                            nullptr);
        ...
        make_set_range_arr =
            (Make_set_range**) bigstack_end_alloc(set_ct * sizeof(intptr_t));
        ...
        while (fgets(g_textbuf, MAXLINELEN, infile)) {
            ...
            *first_token_end       = '\0';
            ...
            if (marker_pos) {
                ...
                if (subset_ct
                    && (bsearch_str(bufptr2, strlen_se(bufptr2),
            ...
            if (scan_uint_defcap(bufptr, &range_first)) {
                sprintf(g_logbuf,
                        "Error: Invalid range start position on line %" PRIuPTR
                        " of %s file.\n",
                        line_idx, file_descrip);
                goto load_range_list_ret_INVALID_FORMAT_2;
            }
            ...
            if (scan_uint_defcap(bufptr, &range_last)) {
                sprintf(g_logbuf,
                        "Error: Invalid range end position on line %" PRIuPTR
                        " of %s file.\n",
                        line_idx, file_descrip);
                goto load_range_list_ret_INVALID_FORMAT_2;
            }
            if (range_last < range_first) {
                sprintf(g_logbuf,
                        "Error: Range end position smaller than range start on "
                        "line %" PRIuPTR " of %s file.\n",
                        line_idx, file_descrip);
                wordwrapb(0);
                goto load_range_list_ret_INVALID_FORMAT_2;
            }
            ...
            if (set_ct > 1) {
                ...
                set_idx = (uint32_t) bsearch_str_natural(
                    bufptr3, set_names, max_set_id_len, set_ct);
            }
            else
            {
                set_idx = 0;
            ...
            if (marker_pos) {
                ...
                range_first = uint32arr_greater_than(&(marker_pos[chrom_start]),
                                                     chrom_end - chrom_start,
                                                     range_first);
                range_last  = uint32arr_greater_than(&(marker_pos[chrom_start]),
                                                    chrom_end - chrom_start,
                                                    range_last + 1);
                if (range_last > range_first) {
                    msr_tmp = (Make_set_range*) bigstack_end_alloc(
                        sizeof(Make_set_range));
                    msr_tmp->next = make_set_range_arr[set_idx];
                    // normally, I'd keep chrom_idx here since that enables
                    // by-chromosome sorting, but that's probably not worth
                    // bloating Make_set_range from 16 to 32 bytes
                              ...
                              Chrom_info* chrom_info_ptr)
{
    ...
    retval = load_range_list(
        infile, 0, 0, 0, 0, 0, allow_no_variants, 0, nullptr, 0, marker_pos,
        chrom_info_ptr, nullptr, nullptr, nullptr, &range_arr, nullptr,
        is_exclude ? "--exclude range" : "--extract range");
    if (retval) {
    ...
    if (is_exclude) {
        while (msr_tmp) {
            fill_bits(msr_tmp->uidx_start,
                      msr_tmp->uidx_end - msr_tmp->uidx_start, marker_exclude);
            msr_tmp = msr_tmp->next;
        }
    }
    else
    {
        ...
        while (msr_tmp) {
            clear_bits(msr_tmp->uidx_start,
                       msr_tmp->uidx_end - msr_tmp->uidx_start,
                       marker_exclude_new);
            msr_tmp = msr_tmp->next;
        }
    ...
    *marker_exclude_ct_ptr =
        popcount_longs(marker_exclude, unfiltered_marker_ctl);
    ...
    {
        orig_marker_exclude_ct =
            *marker_exclude_ct_ptr - orig_marker_exclude_ct;
        LOGPRINTF("--%s range: %" PRIuPTR " variant%s excluded.\n",
                  is_exclude ? "exclude" : "extract", orig_marker_exclude_ct,
                  (orig_marker_exclude_ct == 1) ? "" : "s");
    }
    while (0) {
...
uint32_t save_set_bitfield(uintptr_t* marker_bitfield_tmp, uint32_t marker_ct,
                           uint32_t range_start, uint32_t range_end,
                           uint32_t complement_sets, uint32_t** set_range_pp)
{
    ...
    *set_range_pp = (uint32_t*) g_bigstack_base;
    ...
    save_set_bitfield_standard:
        ...
        memcpy(&((*set_range_pp)[4]),
               &(marker_bitfield_tmp[bound_bottom_d128 / BITCT]), mem_req - 16);
        if (complement_sets) {
            bitarr_invert(bound_top_d128 - bound_bottom_d128,
                          (uintptr_t*) (&((*set_range_pp)[4])));
        }
    }
...
uint32_t save_set_range(uint64_t* range_sort_buf, uint32_t marker_ct,
                        uint32_t rsb_last_idx, uint32_t complement_sets,
                        uint32_t** set_range_pp)
{
    ...
    *set_range_pp = uiptr;
    ...
    {
        if (range_end == marker_ct) {
            if ((((uint32_t)(range_sort_buf[rsb_last_idx] - 1)) / 128)
                < range_ct)
            {
                do_flip     = 1;
                range_start = 0;
    ...
    if (!complement_sets) {
        ...
        ulii *= 16;
        if (ulii > mem_req) {
            fill_ulong_zero((bound_top_d128 - bound_bottom_d128)
                                * (128 / BITCT),
                            bitfield_ptr);
    ...
    else
    {
        ...
        if (ulii > mem_req) {
            ...
            if (do_flip) {
                rsb_last_idx--;
                if (range_start) {
                    // first raw range must begin at bit 0, so complemented
                    // range must begin later
                    uii = range_start;
                    ujj = (uint32_t)(range_sort_buf[0]);
...
int32_t define_sets(Set_info* sip, uintptr_t unfiltered_marker_ct,
                    uintptr_t* marker_exclude, uint32_t* marker_pos,
                    uintptr_t* marker_exclude_ct_ptr, char* marker_ids,
                    uintptr_t max_marker_id_len, Chrom_info* chrom_info_ptr,
                    uint32_t allow_no_variants)
{
    ...
    if (sip->genekeep_flattened) {
        ...
        if (sip->merged_set_name) {
            ...
            while (1) {
                slen = strlen(bufptr);
                if ((slen == uii)
                    && (!memcmp(bufptr, sip->merged_set_name, uii)))
                {
                    break;
                }
        ...
        else
        {
            ...
            if (bigstack_end_alloc_c(genekeep_ct * max_genekeep_len,
                                     &sorted_genekeep_ids))
            {
            ...
            do
            {
                slen = strlen(bufptr) + 1;
                if ((!c_prefix) || (!memcmp(bufptr, "C_", 2))) {
                    memcpy(&(sorted_genekeep_ids[ulii * max_genekeep_len]),
                           bufptr, slen);
                    ulii++;
                }
                bufptr = &(bufptr[slen]);
            } while (*bufptr);
            qsort(sorted_genekeep_ids, genekeep_ct, max_genekeep_len,
                  strcmp_casted);
        }
    }
    ...
    if (sip->setnames_flattened || sip->subset_fname) {
        if (sip->subset_fname) {
            ...
            retval = scan_token_ct_len(MAXLINELEN, infile, g_textbuf,
                                       &subset_ct, &max_subset_id_len);
            if (retval) {
                if (retval == RET_INVALID_FORMAT) {
                    logerrprint(
                        "Error: Pathologically long token in --subset file.\n");
                }
        ...
        if (sip->setnames_flattened) {
            subset_ct += count_and_measure_multistr(sip->setnames_flattened,
                                                    &max_subset_id_len);
        }
        if (!subset_ct) {
            if ((gene_all || sip->genekeep_flattened)
                && ((!sip->merged_set_name) || (!complement_sets)))
            {
                if (!allow_no_variants) {
                    if (sip->subset_fname) {
                        logerrprint("Error: All variants excluded, since "
                                    "--subset file is empty.\n");
                    }
                    else
                    {
                        logerrprint("Error: All variants excluded, since "
                                    "--set-names was given no parameters.\n");
                    }
                    goto define_sets_ret_ALL_MARKERS_EXCLUDED_2;
                }
                else
            ...
            else
            {
                if (sip->subset_fname) {
                    logerrprint(
                        "Warning: Empty --subset file; no sets defined.\n");
                }
                else
                {
                    logerrprint("Warning: No sets defined since --set-names "
                                "was given no parameters.\n");
                }
                goto define_sets_ret_1;
            }
        }
        if (max_subset_id_len > MAX_ID_BLEN) {
            logerrprint("Error: Subset IDs are limited to " MAX_ID_SLEN_STR
                        " characters.\n");
            goto define_sets_ret_INVALID_FORMAT;
        }
        if (bigstack_end_alloc_c(subset_ct * max_subset_id_len,
                                 &sorted_subset_ids))
        {
            goto define_sets_ret_NOMEM;
        }
        if (sip->subset_fname) {
            if (ulii) {
                rewind(infile);
                retval = read_tokens(MAXLINELEN, ulii, max_subset_id_len,
                                     infile, g_textbuf, sorted_subset_ids);
                if (retval) {
                    goto define_sets_ret_1;
                }
            }
        ...
        if (sip->setnames_flattened) {
            ...
            while (*bufptr) {
                slen = strlen(bufptr) + 1;
                memcpy(&(sorted_subset_ids[ulii * max_subset_id_len]), bufptr,
                       slen);
                ulii++;
                bufptr = &(bufptr[slen]);
            }
        ...
        subset_ct = collapse_duplicate_ids(sorted_subset_ids, subset_ct,
                                           max_subset_id_len, nullptr);
    }
    ...
    if (make_set) {
        retval = load_range_list(
            ...
            sip->modifier & SET_MAKE_COLLAPSE_GROUP,
            gene_all || sip->genekeep_flattened, c_prefix, allow_no_variants,
            subset_ct, sorted_subset_ids, max_subset_id_len, marker_pos,
            chrom_info_ptr, &set_ct, &set_names, &max_set_id_len,
            &make_set_range_arr, &range_sort_buf, "--make-set");
        if (retval) {
            goto define_sets_ret_1;
        }
    }
    ...
    if (gene_all || sip->genekeep_flattened) {
        ...
        if (bigstack_end_calloc_ul(unfiltered_marker_ctl, &marker_bitfield_tmp)
            || bigstack_end_alloc_ul(unfiltered_marker_ctl,
                                     &marker_exclude_new))
        {
            goto define_sets_ret_NOMEM;
        }
        fill_all_bits(unfiltered_marker_ct, marker_exclude_new);
        // then include every variant that appears, or include every variant
        // that fails to appear in a fully loaded set in the complement case
        if (make_set) {
            for (set_idx = 0; set_idx < set_ct; set_idx++) {
                if (gene_all
                    || (bsearch_str_nl(&(set_names[set_idx * max_set_id_len]),
                                       sorted_genekeep_ids, max_genekeep_len,
                                       genekeep_ct)
                        != -1))
                {
                    msr_tmp = make_set_range_arr[set_idx];
                ...
                if (complement_sets) {
                    bitvec_and(marker_bitfield_tmp, unfiltered_marker_ctl,
                               marker_exclude_new);
        ...
        {
            if (bigstack_end_alloc_c(marker_ct * max_marker_id_len,
                                     &sorted_marker_ids)
                || bigstack_end_alloc_ui(marker_ct, &marker_id_map))
            {
                goto define_sets_ret_NOMEM;
            }
            retval = sort_item_ids_noalloc(
                unfiltered_marker_ct, marker_exclude, marker_ct, marker_ids,
                max_marker_id_len, 0, 0, strcmp_deref, sorted_marker_ids,
                marker_id_map);
            if (retval) {
                goto define_sets_ret_NOMEM;
            }
            // similar to read_tokens(), since it may be important to support
            // very long lines.
            while (1) {
                ...
                define_sets_tok_start_1:
                    ...
                    if (bufptr2 == &(g_textbuf[MAXLINELEN * 2])) {
                        if (curtoklen > MAXLINELEN) {
                            logerrprint("Error: Excessively long token in "
                                        "--set file.\n");
                            goto define_sets_ret_INVALID_FORMAT;
                        }
                    ...
                    if ((curtoklen == 3) && (!memcmp(bufptr, "END", 3))) {
                        ...
                        if (complement_sets) {
                            bitvec_and(marker_bitfield_tmp,
                                       unfiltered_marker_ctl,
                                       marker_exclude_new);
                            fill_ulong_zero(unfiltered_marker_ctl,
                                            marker_bitfield_tmp);
                        }
                        in_set = 0;
                    }
                    ...
                    {
                        ii = bsearch_str(bufptr, (uintptr_t)(bufptr2 - bufptr),
                                         sorted_marker_ids, max_marker_id_len,
                                         marker_ct);
                        if (ii != -1) {
                            set_bit(marker_id_map[(uint32_t) ii],
                                    marker_bitfield_tmp);
                        }
                    }
                    bufptr = &(bufptr2[1]);
            ...
            if (!set_ct) {
                if (!complement_sets) {
                    if (!allow_no_variants) {
                        logerrprint("Error: All variants excluded by "
                                    "--gene{-all}, since no sets were defined "
                                    "from\n--set file.\n");
                        goto define_sets_ret_ALL_MARKERS_EXCLUDED_2;
                    }
                    else
        ...
        if (!complement_sets) {
            bitvec_andnot(marker_bitfield_tmp, unfiltered_marker_ctl,
                          marker_exclude_new);
        }
        bitvec_or(marker_exclude_new, unfiltered_marker_ctl, marker_exclude);
        marker_exclude_ct =
    ...
    {
        // 5. otherwise, with --set and no --set-collapse-all, count number of
        // sets
        //    and max_name_len.
        while (1) {
            if (fread_checked(midbuf, MAXLINELEN, infile, &bufsize)) {
            ...
            *buf_end   = ' ';
            ...
            define_sets_tok_start_2:
                ...
                else if (!in_set)
                {
                    in_set = 1;
                    if (subset_ct
                        && (bsearch_str(bufptr, (uintptr_t)(bufptr2 - bufptr),
                                        sorted_subset_ids, max_subset_id_len,
                                        subset_ct)
                            == -1))
                    {
                        // no need for in_set = 2, just don't adjust
                        // set_ct/id_len
                        bufptr = &(bufptr2[1]);
                        continue;
                    }
    ...
    fill_uidx_to_idx(marker_exclude, unfiltered_marker_ct, marker_ct,
                     marker_uidx_to_idx);
    if (!set_names) {
        if (sip->merged_set_name) {
            set_ct         = 1;
        ...
        for (set_idx = 0; set_idx < set_ct; set_idx++) {
            ...
            while (msr_tmp) {
                ...
                if (IS_SET(marker_exclude, range_first)) {
                    range_first = next_unset(marker_exclude, range_first,
                ...
                if (IS_SET(marker_exclude, range_last)) {
                    range_last = next_unset(marker_exclude, range_last,
                                            unfiltered_marker_ct);
                }
                ...
                range_sort_buf[uii++] =
                    (((uint64_t) range_first) << 32) | ((uint64_t) range_last);
            }
            if (!uii) {
...
#ifdef __cplusplus
            std::sort((int64_t*) range_sort_buf,
                      (int64_t*) (&(range_sort_buf[uii])));
#else
            ...
            for (ujj = 1; ujj < uii; ujj++) {
                ...
                if (range_first <= range_last) {
                    ...
                    if (umm > range_last) {
                        range_last = umm;
                        range_sort_buf[ukk] =
                            (range_sort_buf[ukk] & 0xffffffff00000000LLU)
                            | (ullii & 0xffffffffLLU);
                    }
                }
                else
            ...
            if (save_set_range(range_sort_buf, marker_ct, ukk, complement_sets,
                               &(all_setdefs[set_idx])))
            {
                goto define_sets_ret_NOMEM;
            }
    ...
    else
    {
        ...
        range_last  = 0;
        // guarantee two free bits at end to simplify loop termination checks
        // (may want to default to doing this...)
        marker_ctp2l = (marker_ct + (BITCT + 1)) / BITCT;
        if (bigstack_end_alloc_ul(marker_ctp2l, &marker_bitfield_tmp)
            || bigstack_end_alloc_c(marker_ct * max_marker_id_len,
                                    &sorted_marker_ids)
            || bigstack_end_alloc_ui(marker_ct, &marker_id_map))
        {
            goto define_sets_ret_NOMEM;
        }
        retval = sort_item_ids_noalloc(unfiltered_marker_ct, marker_exclude,
                                       marker_ct, marker_ids, max_marker_id_len,
                                       0, 1, strcmp_deref, sorted_marker_ids,
                                       marker_id_map);
        if (retval) {
            goto define_sets_ret_1;
        }
        ...
        while (1) {
            ...
            *buf_end   = ' ';
            ...
            define_sets_tok_start_3:
                ...
                curtoklen = (uintptr_t)(bufptr2 - bufptr);
                if ((bufptr2 == buf_end)
                    && (buf_end == &(g_textbuf[MAXLINELEN * 2])))
                {
                    bufptr3 = &(g_textbuf[MAXLINELEN - curtoklen]);
                    memcpy(bufptr3, bufptr, curtoklen);
                    bufptr = bufptr3;
                ...
                if ((curtoklen == 3) && (!memcmp(bufptr, "END", 3))) {
                    ...
                    if ((!sip->merged_set_name) && (in_set == 1)) {
                        if (save_set_bitfield(marker_bitfield_tmp, marker_ct,
                ...
                else if (!in_set)
                {
                    ...
                    if (!sip->merged_set_name) {
                        memcpyx(&(set_names[set_idx * max_set_id_len]), bufptr,
                                bufptr2 - bufptr, '\0');
                    }
                }
                else if (in_set == 1)
        ...
        if (sip->merged_set_name) {
            if (save_set_bitfield(marker_bitfield_tmp, marker_ct, range_first,
                                  range_last + 1, complement_sets, all_setdefs))
            {
                goto define_sets_ret_NOMEM;
            }
        }
...
int32_t write_set(Set_info* sip, char* outname, char* outname_end,
                  uint32_t marker_ct, uintptr_t unfiltered_marker_ct,
                  uintptr_t* marker_exclude, char* marker_ids,
                  uintptr_t max_marker_id_len, uint32_t* marker_pos,
                  Chrom_info* chrom_info_ptr)
{
    unsigned char* bigstack_mark    = g_bigstack_base;
    ...
    if (sip->modifier & SET_WRITE_TABLE) {
        ...
        writebuf[2 * set_ct - 1] = '\n';
        for (marker_idx = 0; marker_idx < marker_ct;
             marker_uidx++, marker_idx++)
        {
            ...
            bufptr =
                chrom_name_write(chrom_info_ptr, chrom_idx, &(g_textbuf[1]));
            *bufptr++ = '\t';
            bufptr    = uint32toa_x(marker_pos[marker_uidx], '\t', bufptr);
            // do not keep double-tab (if it was intentional, it should have
            // been in the header line too...)
    ...
    if (sip->modifier & SET_WRITE_LIST) {
        ...
        fill_idx_to_uidx(marker_exclude, unfiltered_marker_ct, marker_ct,
                         marker_idx_to_uidx);
        for (set_idx = 0; set_idx < set_ct; set_idx++) {
            fputs(&(sip->names[set_idx * max_set_name_len]), outfile);
            putc_unlocked('\n', outfile);
            ...
            if (range_ct != 0xffffffffU) {
                for (uii = 0; uii < range_ct; uii++) {
                    ...
                    for (marker_idx = cur_set_ptr[uii * 2 + 1];
                         marker_idx < ujj; marker_idx++)
                    {
                        fputs(&(marker_ids[marker_idx_to_uidx[marker_idx]
                                           * max_marker_id_len]),
                              outfile);
                        putc_unlocked('\n', outfile);
                    }
            ...
            else
            {
                ...
                if ((range_start + uii < marker_ct) && cur_set_ptr[3]) {
                    for (marker_idx = range_start + uii; marker_idx < marker_ct;
                         marker_idx++)
                    {
                        fputs(&(marker_ids[marker_idx_to_uidx[marker_idx]
                                           * max_marker_id_len]),
...
void unpack_set(uintptr_t marker_ct, uint32_t* setdef,
                uintptr_t* include_bitfield)
{
    ...
    if (range_ct == 0xffffffffU) {
        ...
        memcpy(&(include_bitfield[range_start / BITCT]),
               (uintptr_t*) (&(setdef[4])), ((range_ct + 127) / 128) * 16);
        ...
        if (uii < marker_ct) {
            ...
            {
                fill_ulong_zero(marker_ctl - uii / BITCT,
                                &(include_bitfield[uii / BITCT]));
            }
        }
    }
    else
    {
        ...
        for (uii = 0; uii < range_ct; uii++) {
            ...
            fill_bits(range_start, setdef[uii * 2 + 2] - range_start,
                      include_bitfield);
        }
    }
}

void unpack_set_unfiltered(uintptr_t marker_ct, uintptr_t unfiltered_marker_ct,
                           uintptr_t* marker_exclude, uint32_t* setdef,
                           uintptr_t* new_exclude)
{
    uintptr_t  unfiltered_marker_ctl = BITCT_TO_WORDCT(unfiltered_marker_ct);
    ...
    uint32_t   range_start;
    uint32_t   range_idx;
    memcpy(new_exclude, marker_exclude,
    ...
    if (range_ct == 0xffffffffU) {
        ...
        if (range_start) {
            ...
            marker_uidx = jump_forward_unset_unsafe(marker_exclude,
                                                    last_uidx + 1, range_start);
            if (!keep_outer) {
        ...
        for (range_idx = 0; range_idx < range_ct; range_idx++, marker_uidx++) {
            next_unset_ul_unsafe_ck(marker_exclude, &marker_uidx);
            // we know that range representation is not more compact, so
            // probably not worthwhile to use next_unset/next_set/fill_bits()
            // here
        ...
        if ((!keep_outer) && (range_start + range_ct < marker_ct)) {
            fill_bits(marker_uidx, unfiltered_marker_ct - marker_uidx,
                      new_exclude);
        }
    }
    else
    {
        ...
        for (; range_idx < range_ct; range_idx++) {
            ...
            if (range_start > range_end) {
                marker_uidx = jump_forward_unset_unsafe(
                    marker_exclude, last_uidx + 1, range_start - range_end);
            }
            fill_bits(last_uidx, marker_uidx - last_uidx, new_exclude);
            next_unset_ul_unsafe_ck(marker_exclude, &marker_uidx);
        unpack_set_unfiltered_late_start:
            ...
            last_uidx = jump_forward_unset_unsafe(
                marker_exclude, marker_uidx + 1, range_end - range_start);
        }
        fill_bits(last_uidx, unfiltered_marker_ct - last_uidx, new_exclude);
    }
}

uint32_t extract_set_union(uint32_t** setdefs, uintptr_t set_ct,
                           uintptr_t* set_incl, uintptr_t* filtered_union,
                           uintptr_t marker_ct)
{
    uintptr_t marker_ctl = BITCT_TO_WORDCT(marker_ct);
    ...
    for (set_idx = 0; set_idx < set_ct; set_idx++) {
        ...
        if (range_ct == 0xffffffffU) {
            ...
            if (range_end > unset_startw) {
                ...
                if (range_start < range_end) {
                    bitvec_or((uintptr_t*) (&(
                                  cur_setdef[4 + (BITCT / 32) * read_offset])),
                              range_end - range_start,
                              &(filtered_union[range_start]));
                }
            }
            if (keep_outer && (range_end < unset_endw)) {
                // may overfill end
                fill_ulong_one(unset_endw - range_end,
                               &(filtered_union[range_end]));
        ...
        else if (range_ct)
        {
            ...
            if (unset_startw) {
                ...
                read_offset = uint32arr_greater_than(cur_setdef, range_ct * 2,
                                                     unset_startw * BITCT + 1)
                              / 2;
                if (read_offset) {
            ...
            if (unset_endw < marker_ctl) {
                ...
                range_ct = (uint32arr_greater_than(cur_setdef, range_ct * 2,
                                                   unset_endw * BITCT)
                            + 1)
                           / 2;
            }
            if (range_ct) {
                ...
                if (range_ct > 1) {
                    fill_bits(range_start, range_end - range_start,
                              filtered_union);
                                      ...
                                      uintptr_t*  union_marker_ct_ptr)
{
    ...
    // point to marker_exclude.  Otherwise, allocates union_marker_exclude on
    // the "stack". Assumes marker_ct is initial value of *union_marker_ct_ptr.
    uintptr_t  unfiltered_marker_ctl = BITCT_TO_WORDCT(unfiltered_marker_ct);
    ...
    *union_marker_ct_ptr = extract_set_union(sip->setdefs, sip->ct, set_incl,
                                             filtered_union, orig_marker_ct);
    ...
    {
        uncollapse_copy_flip_include_arr(filtered_union, unfiltered_marker_ct,
                                         marker_exclude, union_marker_exclude);
...
{
    // currently assumes marker_exclude does not exclude anything in the union
    ...
    fill_midx_to_idx(marker_exclude_orig, marker_exclude, marker_ct,
                     marker_midx_to_idx);
        ...
        if (range_ct != 0xffffffffU) {
            if (range_ct) {
                ...
                for (range_idx = 0; range_idx < range_ct; range_idx++) {
                    ...
                    fill_bits(marker_midx_to_idx[range_offset],
                              range_stop - range_offset, cur_bitfield);
                }
                range_end = marker_midx_to_idx[range_offset] + range_stop
                            - range_offset;
            }
        }
        else
        {
            ...
            for (marker_midx = 0; marker_midx < range_stop; marker_midx++) {
                if (IS_SET(read_bitfield, marker_midx)) {
                    set_bit(marker_midx_to_idx[marker_midx + range_offset],
                            cur_bitfield);
                }
            }
            if (include_out_of_bounds
                && (range_offset + range_stop < marker_ct_orig))
            {
                fill_bits(marker_midx_to_idx[range_offset + range_stop],
                          marker_ct_orig - range_offset - range_stop,
                          cur_bitfield);
                range_end = marker_ct;
            }
            else
            {
                range_end =
                    1 + last_set_bit(cur_bitfield, BITCT_TO_WORDCT(marker_ct));
            }
            if (range_start) {
                range_start =
                    marker_midx_to_idx[next_set_unsafe(read_bitfield, 0)
                                       + range_offset];
            }
        }
        if (save_set_bitfield(cur_bitfield, marker_ct, range_start, range_end,
                              0, &(new_setdefs[set_idx])))
        {
            bigstack_end_reset(bigstack_end_mark);
            return 1;
        }
...
int32_t load_range_list_sortpos(
    char* fname, uint32_t border_extend, uintptr_t subset_ct,
    char* sorted_subset_ids, uintptr_t max_subset_id_len,
    Chrom_info* chrom_info_ptr, uintptr_t* gene_ct_ptr, char** gene_names_ptr,
    uintptr_t* max_gene_id_len_ptr, uintptr_t** chrom_bounds_ptr,
    uint32_t*** genedefs_ptr, uintptr_t* chrom_max_gene_ct_ptr,
    const char* file_descrip)
{
    ...
    uint32_t       chrom_code_end =
        chrom_info_ptr->max_code + 1 + chrom_info_ptr->name_ct;
    uint32_t         chrom_idx = 0;
    ...
    retval = load_range_list(infile, 1, border_extend, 0, 0, 0, 0, subset_ct,
                             sorted_subset_ids, 0, nullptr, chrom_info_ptr,
                             &gene_ct, gene_names_ptr, &max_gene_id_len,
                             &gene_arr, &range_sort_buf, file_descrip);
    if (retval) {
    ...
    for (gene_idx = 0; gene_idx < gene_ct; gene_idx++) {
        ...
        // (last prefix character must be nonnumeric to prevent weird natural
        // sort interaction)
        uii = (((unsigned char) bufptr[0]) * 1000)
              + (((unsigned char) bufptr[1]) * 100)
              + (((unsigned char) bufptr[2]) * 10) + ((unsigned char) bufptr[3])
              - 53313;
        if (chrom_idx < uii) {
        ...
        uii     = 0;
        while (msr_tmp) {
            range_sort_buf[uii++] = (((uint64_t)(msr_tmp->uidx_start)) << 32)
                                    | ((uint64_t)(msr_tmp->uidx_end));
            msr_tmp = msr_tmp->next;
        }
        if (!uii) {
            if (bigstack_left() < 16) {
...
int32_t gene_report(char* fname, char* glist, char* subset_fname,
                    uint32_t border, char* extractname, const char* snp_field,
                    char* outname, char* outname_end, double pfilter,
                    Chrom_info* chrom_info_ptr)
{
    ...
    if (subset_fname) {
        ...
        retval = scan_token_ct_len(MAXLINELEN, infile, g_textbuf, &subset_ct,
                                   &max_subset_id_len);
        if (retval) {
            if (retval == RET_INVALID_FORMAT) {
                logerrprint("Error: Pathologically long token in --gene-subset "
                            "file.\n");
            }
        ...
        if (max_subset_id_len > MAX_ID_BLEN) {
            logerrprint(
                "Error: --gene-subset IDs are limited to " MAX_ID_SLEN_STR
                " characters.\n");
            goto gene_report_ret_INVALID_FORMAT;
        }
        if (bigstack_end_alloc_c(subset_ct * max_subset_id_len,
        ...
        rewind(infile);
        retval = read_tokens(MAXLINELEN, subset_ct, max_subset_id_len, infile,
                             g_textbuf, sorted_subset_ids);
        if (retval) {
            goto gene_report_ret_1;
        }
        if (fclose_null(&infile)) {
    ...
    if (extractname) {
        ...
        retval = scan_token_ct_len(MAXLINELEN, infile, g_textbuf, &extract_ct,
                                   &max_extract_id_len);
        if (retval) {
            goto gene_report_ret_1;
        }
        ...
        if (max_extract_id_len > MAX_ID_BLEN) {
            logerrprint("Error: --extract IDs are limited to " MAX_ID_SLEN_STR
                        " characters.\n");
            goto gene_report_ret_INVALID_FORMAT;
        }
        if (bigstack_alloc_c(extract_ct * max_extract_id_len,
                             &sorted_extract_ids))
        {
            goto gene_report_ret_NOMEM;
        }
        ...
        retval = read_tokens(MAXLINELEN, extract_ct, max_extract_id_len, infile,
                             g_textbuf, sorted_extract_ids);
        if (retval) {
            goto gene_report_ret_1;
        }
        if (fclose_null(&infile)) {
        ...
        qsort(sorted_extract_ids, extract_ct, max_extract_id_len,
              strcmp_casted);
        ulii = collapse_duplicate_ids(sorted_extract_ids, extract_ct,
                                      max_extract_id_len, nullptr);
        if (ulii < extract_ct) {
            extract_ct = ulii;
            bigstack_shrink_top(sorted_extract_ids,
                                extract_ct * max_extract_id_len);
        }
    }
    retval = load_range_list_sortpos(
        glist, 0, subset_ct, sorted_subset_ids, max_subset_id_len,
        chrom_info_ptr, &gene_ct, &gene_names, &max_gene_name_len,
        &chrom_bounds, &genedefs, &chrom_max_gene_ct, "--gene-report");
    if (retval) {
        goto gene_report_ret_1;
    }
#ifdef __LP64__
    ...
    for (gene_idx = 0; gene_idx < gene_ct; gene_idx++) {
        ...
        bufptr2 = memcpyax(&(loadbuf[gene_idx * max_gene_name_len]),
                           &(bufptr[4]), slen, ' ');
        memcpyx(bufptr2, &(bufptr[2]), 2, '\0');
    }
    if (qsort_ext(loadbuf, gene_ct, max_gene_name_len, strcmp_natural_deref,
                  (char*) gene_nameidx_to_chridx, sizeof(int32_t)))
    {
        goto gene_report_ret_NOMEM;
    }
    ...
    retval = open_and_load_to_first_token(&infile, fname, loadbuf_size, '\0',
                                          "--gene-report file", loadbuf,
                                          &bufptr, &line_idx);
    if (retval) {
        goto gene_report_ret_1;
    }
    do
    {
        ...
        if (slen <= uii) {
            ...
            else if ((slen == snp_field_len)
                     && (!memcmp(bufptr, snp_field, snp_field_len)))
            {
                ujj = 2;
            }
            ...
            if (ujj != 4) {
                if ((found_header_bitfield >> ujj) & 1) {
                    *bufptr2 = '\0';
                    sprintf(g_logbuf,
                            "Error: Duplicate column header '%s' in %s.\n",
                            bufptr, fname);
                    goto gene_report_ret_INVALID_FORMAT_WW;
                }
    ...
    while (1) {
        // store raw line contents at bottom of stack (growing upwards), and
        // gene matches at top of stack (8 byte entries, growing downwards).
        // Worst case, a line matches chrom_max_gene_ct genes, so we prevent the
        // text loading buffer from using the last chrom_max_gene_ct * 8 bytes
        // of workspace memory.
        if (linebuf_left <= chrom_max_gene_ct * 8 + 8
                                + (uintptr_t)(((char*) gene_match_list_end)
                                              - ((char*) gene_match_list))
                                + MAXLINELEN)
        {
            goto gene_report_ret_NOMEM;
        }
        loadbuf_size = linebuf_left - (chrom_max_gene_ct * 8) - 8
                       - (uintptr_t)(((char*) gene_match_list_end)
                                     - ((char*) gene_match_list));
        if (loadbuf_size > MAXLINEBUFLEN) {
            loadbuf_size = MAXLINEBUFLEN;
    ...
    gene_report_load_loop:
        ...
        // CHR
        chrom_idx = get_chrom_code_counted(
            chrom_info_ptr, strlen_se(token_ptrs[0]), token_ptrs[0]);
        if (chrom_idx < 0) {
            // todo: log warning?
            goto gene_report_load_loop;
        }
        ...
        if (sorted_extract_ids) {
            ...
            if (bsearch_str(bufptr2, strlen_se(bufptr2), sorted_extract_ids,
                            max_extract_id_len, extract_ct)
                == -1)
            {
                goto gene_report_load_loop;
            }
        ...
        ujj = cur_bp + border;
        for (gene_idx = chrom_bounds[(uint32_t) chrom_idx]; gene_idx < ulii;
             gene_idx++)
        {
            if (interval_in_setdef(genedefs[gene_idx], uii, ujj)) {
                *(--gene_match_list) =
                    (((uint64_t) gene_chridx_to_nameidx[gene_idx]) << 32)
                    | saved_line_ct;
            }
        }
        ...
        *((uint32_t*) linebuf_top)   = slen;
        ...
        if (saved_line_ct == 0x100000000LLU) {
            sprintf(g_logbuf,
                    "Error: Too many valid lines in %s (--gene-report can only "
                    "handle 4294967296).\n",
                    fname);
            goto gene_report_ret_INVALID_FORMAT_WW;
        }
    ...
    if (linebuf_left < saved_line_ct * sizeof(intptr_t)
                           + (uintptr_t)(((char*) gene_match_list_end)
                                         - ((char*) gene_match_list)))
    {
        goto gene_report_ret_NOMEM;
    ...
    line_lookup = (char**) (&(
        ((uintptr_t*)
             gene_match_list)[-((intptr_t)((uintptr_t) saved_line_ct))]));
...
#else
    qsort((int64_t*) gene_match_list,
          (uintptr_t)(gene_match_list_end - gene_match_list), sizeof(int64_t),
          llcmp);
#endif
    ...
    while (gene_match_list < gene_match_list_end) {
        ...
        bufptr2 = dtoa_g_wxp4(((double) ((int32_t)(ujj - cur_bp))) * 0.001, 10,
                              g_textbuf);
