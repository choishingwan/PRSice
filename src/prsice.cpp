                           ...
                           Reporter& reporter)
{
    ...
    if (input_sanity_check.size() < 2 && !binary && regress)
    {
        ...
        if ((*itr) == -9) { message.append(" and they are all -9"); }
        reporter.report(message);
                                          ...
                                          Reporter& reporter)
{
    ...
    else
    {
        ...
        for (auto cov : cov_header)
        {
            ...
            if (included.find(cov) == included.end()) { included.insert(cov); }
        }
    ...
    if (cov_index.size() == 0)
    { throw std::runtime_error("ERROR: No valid covariates!"); } else
    {
        std::string message = "";
        if (cov_index.size() == 1)
    ...
    while (std::getline(cov, line))
    {
        ...
        { // sample is found in the phenotype vector
            for (size_t i_cov = 0; i_cov < cov_index.size(); ++i_cov)
            {
                ...
                { current_factors[i_cov][token[covar_index]]++; } else
                {
                        ...
                        Genotype& target)
{
    ...
    if (all)
    {
        ...
        if (multi) { all_out_name.append("." + pheno_info.name[pheno_index]); }
...
void PRSice::permutation(const size_t n_thread, bool logit_perm)
{
    ...
    for (int iter = 0; iter < num_iter + 1; ++iter)
    {
        ...
        { cur_perm = m_num_perm - processed; } cur_remain--;
    ...
    int rank, const Eigen::VectorXd& pre_se, size_t processed, bool logit_perm)
{
    ...
    for (size_t i = start; i < end; ++i)
    {
        ...
        else
        {
            ...
            int rdf = n - rank;
            double rss = 0.0;
            for (size_t r = 0; r < n; ++r) { rss += residual(r) * residual(r); }
...
void PRSice::summarize(const Commander& commander, Reporter& reporter)
{
    ...
    if (m_significant_store[1] != 0)
    {
        ...
        { message.append(" and "); } message.append(
