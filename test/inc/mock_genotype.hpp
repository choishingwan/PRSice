...
public:
   ...
   bool test_parse_chr(const std::vector<std::string_view>& token,
                      const BaseFile& base_file, std::vector<size_t> &filter_count, size_t& chr){
       return parse_chr(token, base_file, filter_count, chr);
   }
   void test_init_chr(int num_auto=22, bool no_x=false, bool no_y=false, bool no_xy=false, bool no_mt=false){
   ...
   test_transverse_base_file(
       ...
       return transverse_base_file(
               base_file,  base_qc,
              threshold_info,
              exclusion_regions,
              file_length, gz_input,
                  std::move(input));
   }
   ...
   bool test_parse_pvalue(const std::string_view& p_value_str,
                         const double max_threshold,std::vector<size_t> &filter_count, double& pvalue){
       return parse_pvalue(p_value_str, max_threshold, filter_count, pvalue);
   }
   bool test_parse_stat(const std::string_view& stat_str, const bool odd_ratio,std::vector<size_t>& filter_count,
                       double& stat){
   ...
   bool test_base_filter_by_value(const std::vector<std::string_view>& token,
                             const BaseFile& base_file,
                                  const double& threshold, std::vector<size_t>&filter_count, size_t type, size_t index){
       return(base_filter_by_value(token, base_file, threshold, filter_count, type, index));
   }
   bool test_parse_rs_id(const std::vector<std::string_view>& token,
                        const BaseFile& base_file, std::unordered_set<std::string>& processed_rs,std::unordered_set<std::string>& dup_index, std::vector<size_t> &filter_count, std::string& rs_id){
       return parse_rs_id(token, base_file, processed_rs, dup_index, filter_count, rs_id);
   }
   unsigned long long
   test_cal_bar_category(const double& pvalue,
                          const std::vector<double>& barlevels, double& pthres){
       return cal_bar_category(pvalue,
                                 barlevels,pthres);
   }
   unsigned long long test_calculate_category(const PThresholding& thresholding,
   ...
   std::unordered_map<std::string, size_t> existed_snps_idx() const { return m_existed_snps_index;}
