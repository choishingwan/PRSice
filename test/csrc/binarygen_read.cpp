...
TEST_CASE("read bgen genotype for LD")
{
    ...
    std::vector<uintptr_t> expected_geno, tmp1, tmp2;
    ...
    mock_binarygen::generate_samples(n_entries, n_target, qc, tmp1, target_prob,
                                     target_founder, std::get<0>(settings),
                                     std::get<1>(settings));
    std::vector<double> ref_prob(n_ref * n_entries);
    std::vector<double> second_prob(n_ref * n_entries);
    mock_binarygen::generate_samples(n_entries, n_ref, qc, tmp2, ref_prob,
                                     ref_founder, std::get<0>(settings),
                                     std::get<1>(settings));
    // we want read_genotype to read the second SNP just in case
    mock_binarygen::generate_samples(
        n_entries, n_ref, qc, expected_geno, second_prob, ref_founder,
    ...
    const uintptr_t unfiltered_sample_ctl = BITCT_TO_WORDCT(n_ref);
    const uintptr_t unfiltered_sample_ctv2 = 2 * unfiltered_sample_ctl;
    std::vector<uintptr_t> observed_geno(unfiltered_sample_ctv2, 0);
    auto target_snps = target_bgen.existed_snps();
    ref_bgen.test_read_genotype(observed_geno.data(), target_snps.back());
